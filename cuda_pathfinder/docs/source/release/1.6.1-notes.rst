...
* Correct Windows binary discovery for separately installed Nsight Systems and
  Nsight Compute tools, the CUDA Compute Sanitizer layout, and utilities that
  are unavailable on Windows. Nsight Systems selects the native machine binary
  even when Python runs under emulation.
